...
#### Scenario: Custom recurrence uses daily periods
- **WHEN** task has Custom recurrence (specific dates like Feb 25, Feb 28, Mar 5)
- **THEN** each custom date is tracked as a separate period
- **AND** period_start equals the custom date being completed

#### Scenario: OneTime uses no period grouping
- **WHEN** task has OneTime recurrence
- **THEN** no period grouping is applied (all-time period)
